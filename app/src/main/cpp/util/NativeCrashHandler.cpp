    ...
    if (result != JNI_OK)
        ...
        if (unwind_backtrace_signal_arch != NULL && siginfo != NULL) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                Verify(env->ExceptionCheck() == JNI_FALSE, "Java threw an exception1");
            }
        ...
        Verify(env->ExceptionCheck() == JNI_FALSE, "Java threw an exception2");
    }
    else
        __android_log_print(ANDROID_LOG_ERROR, "NativeCrashHandler",
...
Java_com_sparkfengbo_ng_livestreamdemoproject_nativehandler_NativeCrashHandler_nRegisterForNativeCrash(JNIEnv *env,
                                                                                                       jobject instance) {

    if (applicationClass) {
        applicationObject = (jobject) env->NewGlobalRef(instance);
...
Java_com_sparkfengbo_ng_livestreamdemoproject_nativehandler_NativeCrashHandler_nUnregisterForNativeCrash(JNIEnv *env,
                                                                                                         jobject instance) {
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    applicationClass = env->FindClass(
            "com/sparkfengbo/ng/livestreamdemoproject/nativehandler/NativeCrashHandler");
    ...
    Verify(env->ExceptionCheck() == JNI_FALSE, "Java threw an exception3");
