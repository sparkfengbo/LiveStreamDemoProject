 ...
 * @param config
 */
AACEncoder::AACEncoder(AudioConfig *config) {
...
int AACEncoder::initAAEncoder() {
    ...
    if (recordConfig->isFileOut && recordConfig->audio_out_path != NULL) {
        size_t path_length = strlen(recordConfig->audio_out_path);
        char *outfile = (char *) malloc(path_length + 1);
        strcpy(outfile, recordConfig->audio_out_path);
...
//    pCodec = avcodec_find_encoder(AV_CODEC_ID_AAC);
    ...
    */
    ...
    pCodecCtx->sample_rate = recordConfig->audio_sample_rate;
...
//    LOGI("channels : %d ", pCodecCtx->channels);
    ...
    if (state < 0) {
        ...
        LOGE("error open audio encoder ret = %d %s", state, buf);
        return -1;
    } else {
    ...
    pFrame->channel_layout = av_get_channel_layout_nb_channels(AV_CH_LAYOUT_MONO);
    pFrame->format = pCodecCtx->sample_fmt;
...
//    frame_size = av_samples_get_buffer_size(NULL, pCodecCtx->channels, pCodecCtx->frame_size, pCodecCtx->sample_fmt, 1);
    ...
    frame_size = av_samples_get_buffer_size(NULL, pCodecCtx->channels, pCodecCtx->frame_size, pCodecCtx->sample_fmt, 0);
...
//    audioFrameData->data = new_buf;
//    audioFrameData->pts = av_gettime();
...
int AACEncoder::encodeAAC(FrameData *data) {
    int ret = 0;

    ret = avcodec_fill_audio_frame(pFrame, pCodecCtx->channels, pCodecCtx->sample_fmt, data->data, frame_size, 0);
    if (ret < 0) {
        char buf[1024] = {0};
        av_strerror(ret, buf, sizeof(buf));
        LOGE("AACEncoder encodeAAC avcodec_fill_audio_frame faile %d %s", ret, buf);
        return -1;
    }

    LOGI("AACEncoder encodeAAC avcodec_fill_audio_frame success");
    ...
    LOGI("AACEncoder encodeAAC avcodec_encode_audio2 success");
